...
class FullLocaleMiddleware(MiddlewareMixin):
    """request should be authenticated before this"""
    ...
    def process_request(self, request):
        ...
        if not language_from_path and not prefixed_default_language:
            if user_profile is None:
                language = settings.LANGUAGE_CODE
            else:
                language = user_profile.preferred_language or user_profile.latest_language
        translation.activate(language)
        if user_profile is not None and user_profile.latest_language != language:
